...
async function regexSearch() {
    ...
    for (module of result) {
        regexSearchResults.innerHTML += `<li class="list-group-item">
            <a class="text-decoration-none" href="/module/${module.url}${module.id ? `?${module.id}=${b64_query}` : ""}">
                ${module.name} 
                ${
                    module.element_name ? `<span class="text-secondary description"> > ${module.element_name}</span>` : ""
                }
            </a>
        </li>`
    }
